...
class EdgeFunctionTester {
  constructor() {
    ...
    console.log(`🧪 TESTE DA EDGE FUNCTION - ${this.executionId}`);
  ...
  async testEdgeFunction() {
    console.log('🚀 TESTANDO EDGE FUNCTION: download-hitss-edge');
    await this.log('EDGE_TEST', 'INICIADO', 'Testando Edge Function de download');
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('download-hitss-edge');
      ...
      await this.log('EDGE_TEST', 'SUCESSO',
        `Download: ${data.downloadTime?.toFixed(2)}s, Total: ${data.totalTime?.toFixed(2)}s, Arquivo: ${data.fileName}`);
  ...
  async execute() {
    console.log('🧪 EXECUTANDO TESTE DA EDGE FUNCTION\n');
...
async function main() {
  try {
    ...
    console.log(`\n${result ? '🎉' : '💥'} Teste da Edge Function ${result ? 'concluído com sucesso' : 'falhou'}`);
